...
class PPS5(Problem):
  ...
  def evaluate(self, decisions):
    ...
    for j in xrange(1, n):
      if is_even(j+1):
        val = (x[j] - (0.3*(x[0]**2)*cos(24*PI*x[0]+4*(j+1)*PI/n) + 0.6*x[0])*sin(6*PI*x[0] + (j+1)*PI/n)) ** 2
        j2_values.append(val)
      else:
        val = (x[j] - (0.3*(x[0]**2)*cos(24*PI*x[0]+4*(j+1)*PI/n) + 0.6*x[0])*cos(6*PI*x[0] + (j+1)*PI/n)) ** 2
  ...
  def get_pareto_front(self):
    file_name = "problems/pps/PF/pps5.txt"
    pf = []
